  ...
  return (
    <div className="min-h-screen flex flex-col justify-center px-8 bg-black relative overflow-hidden">
      ...
      <div className="mb-12 text-center animate-slide-up">
        <div className="w-20 h-20 pink-gradient rounded-[2.2rem] mx-auto mb-8 flex items-center justify-center text-white text-4xl font-black shadow-[0_0_50px_rgba(255,45,85,0.3)]">L</div>
        <h1 className="text-4xl font-black tracking-tighter text-white">freeLynk</h1>
        <p className="text-zinc-500 font-bold uppercase tracking-[0.25em] text-[10px] mt-2">Pink Edition Secure Login</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-sm mx-auto w-full relative z-10">
        ...
        <button type="submit" disabled={isLoggingIn} className="w-full pink-gradient h-14 rounded-2xl text-white font-black uppercase tracking-[0.2em] text-xs shadow-xl active:scale-95 transition-all">
      ...
      <button onClick={onToggle} className="mt-12 text-zinc-500 text-xs font-bold w-full text-center uppercase tracking-widest">
        New to Network? <span className="text-[#FF2D55] font-black">Register</span>
      </button>
    </div>
  );
};
