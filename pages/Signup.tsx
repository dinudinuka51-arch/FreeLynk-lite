...
const Signup: React.FC<SignupProps> = ({ onSignup, onToggle }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authData.user) {
        await supabase.from('profiles').insert([{ id: authData.user.id, uid: tempUid, name, profile_photo: photo, bio: 'Pink Network Member' }]);
    ...
    <div className="min-h-screen flex flex-col justify-center px-8 bg-black py-10 relative overflow-hidden">
      ...
      <div className="mb-10 text-center flex flex-col items-center animate-slide-up">
        <div className="relative mb-6">
          ...
          <label className="absolute bottom-1 right-1 pink-gradient p-2 rounded-full border-2 border-black cursor-pointer shadow-lg">
        ...
        <span className="text-[#FF2D55] font-mono text-[9px] uppercase tracking-widest mt-2 font-black bg-pink-500/10 px-3 py-1 rounded-full border border-pink-500/20">{tempUid}</span>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-sm mx-auto w-full relative z-10">
        ...
        <button type="submit" disabled={isVerifying} className="w-full h-14 rounded-2xl pink-gradient text-white font-black uppercase tracking-[0.2em] text-xs shadow-xl active:scale-95 transition-all">
      ...
      <button onClick={onToggle} className="mt-8 text-zinc-500 text-xs font-bold w-full text-center uppercase tracking-widest">
        Active ID? <span className="text-[#FF2D55] font-black">Login</span>
      </button>
    </div>
  );
};
